...
| 真机与公开发行 | 部分通过 / 阻塞 | iPhone 一次性命令、暂停/恢复、UIKit 前后台、强制重启持久化、受限存储故障和有界内存警告恢复已通过；最大实验组合 inventory/SPDX SBOM 已生成，仍需真实 storage pressure、iPad、jetsam/断电、最终制品扫描与完整发行物 SBOM、许可证/NOTICE/对应源码和 App Store 审查 |
...
PocketRoot 自身许可证仍在首个公开版本前确认中。实验性运行时链接 GPL 标识的上游代码，候选 RootFS 包含多种 copyleft 与 permissive 许可证。RootFS 包级和最大实验工程组合 SPDX SBOM 已生成，但生产、TestFlight 和公开二进制分发保持关闭，直到完整真机生命周期、许可证、NOTICE、对应源码、最终制品扫描后的完整发行物 SBOM 和 App Store Review Guideline 2.5.2 均有明确结论。
