...
if [[ ! "$STABILITY_ITERATIONS" =~ ^[0-9]+$ ]] \
  || [[ "$STABILITY_ITERATIONS" -lt 20 ]] \
  || [[ "$STABILITY_ITERATIONS" -gt 600 ]]; then
    echo "POCKETROOT_SMOKE_STABILITY_ITERATIONS must be an integer from 20 through 600." >&2
    exit 2
fi
if [[ ! "$STABILITY_INTERVAL_MILLISECONDS" =~ ^[0-9]+$ ]] \
  || [[ "$STABILITY_INTERVAL_MILLISECONDS" -lt 25 ]] \
  || [[ "$STABILITY_INTERVAL_MILLISECONDS" -gt 10000 ]]; then
...
SIMCTL_CHILD_POCKETROOT_SMOKE_STABILITY="$STABILITY_MODE" \
SIMCTL_CHILD_POCKETROOT_SMOKE_STABILITY_ITERATIONS="$STABILITY_ITERATIONS" \
SIMCTL_CHILD_POCKETROOT_SMOKE_STABILITY_INTERVAL_MILLISECONDS="$STABILITY_INTERVAL_MILLISECONDS" \
xcrun simctl launch \
  --console \
