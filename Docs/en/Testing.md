...
- Core tests cover request/configuration defaults, result-stream decoding, placeholder lifecycle behavior, injected-runtime delegation and shutdown-failure state refresh, plus RootFS-provider/metadata coordination.
- Resources tests cover the pinned manifest and gated bundled provider; archive existence, symlink rejection, byte count, and SHA-256; required fakefs layout and metadata-symlink rejection; successful gzip/ustar extraction, traversal cleanup, archive symlink entry/source rejection, and the expanded-byte limit.
...
- Injected runtime tests cover configuration and host availability, missing/symlinked fakefs preflight, boot and `/bin/sh -lc` request mapping, execute-before-boot, timeout validation/clamping, process ownership, failed/concurrent boot admission, active-command shutdown ordering, output-limit mapping, and terminated/`restartRequired` behavior.
