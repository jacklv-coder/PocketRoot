...
| License-reviewed RootFS | Blocked | The 15-package/10-origin evidence, corresponding-source candidate material for all 10 origins, all 78 initial candidates, and all 138 external LICENSE/NOTICE payloads have checksum-bound engineering review; only the `alpine-keys` MIT grant/copyright notice remains open, followed by a complete NOTICE, rebuild environment/toolchain, source-offer mechanics, legal review, delivery approval, and authorized release |
