...
| Host App UI smoke | `./Scripts/run-host-app-ui-smoke.sh` | Apple Silicon + iOS 18 Simulator + archive | 公开宿主手动与一体化自动 Boot、SwiftTerm PTY 输入/持续输出、前后台、旋转 resize、关闭/重开、Workspace 跨页面 PTY 持续性、Files 预览与有序 shutdown | 真机键盘、iPad 或发行可用 |
...
smoke，并运行 Host App 的 PTY 输入、文件创建、Files 导航与预览 UI 闭环。CI 的
Simulator 结果不证明签名真机或发行可用。
