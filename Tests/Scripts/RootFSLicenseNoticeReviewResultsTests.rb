...
class RootFSLicenseNoticeReviewResultsTests < Minitest::Test
  ...
  def test_validates_pinned_candidate_review_results
    ...
    assert_equal 94, @results.fetch("reviewedPayloadFileCount")
    assert_equal 5,
  ...
  def test_binds_enabled_busybox_bzip2_to_its_exact_license
    source = @results.fetch("sources").find do |candidate|
  ...
  def test_binds_enabled_busybox_ash_math_to_inline_notices
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
    ...
    assert_equal "partial", source.fetch("licenseTextCoverage")
    assert_equal "partial", source.fetch("attributionCoverage")
    assert_equal 16, source.fetch("reviewedExistingEvidenceCount")
    assert_includes(
      source.fetch("resolvedReviewItems"),
  ...
  def test_binds_enabled_busybox_env_to_inline_notice
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_echo_to_inline_notice
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_logger_to_inline_notice
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_cal_to_inline_notice
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_traceroute_and_traceroute6_to_inline_notice
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_od_hexdump_and_hd_to_inline_notices
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
  ...
  def test_binds_enabled_busybox_expand_unexpand_and_fold_to_license_headers
    source = @results.fetch("sources").find do |candidate|
      candidate.fetch("sourceOrigin") == "busybox"
    end
