...
PocketRoot 的重要变化记录在这里，并从首个公开版本开始遵循 Semantic Versioning。
...
- 将签名真机 3 分钟持续负载 smoke 升级为 Simulator/真机共用、可配置的稳定性门禁：
  一个 PTY 保持 20...600 轮，每 10 轮以唯一边界流过 64 KiB 零字节 payload 并逐字节
  校验长度和内容，同时交叉验证一次性命令与 Files；
...
- 新增 `v0.1.0` 源码 tag 审计：在指定 Git ref 的隔离快照内核对版本授权、发布文档
...
- Host App iPad 文件导入 UI smoke 会先把目标文件行滚动到 List 的有效可见区域，
  再按当前 frame 执行点击或长按，避免 Xcode 16 对离屏 SwiftUI 行计算
  `isHittable` 时因无效 activation point 产生假失败。
